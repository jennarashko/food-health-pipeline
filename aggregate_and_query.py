...
def _build_flatten_and_label_pipeline() -> list:
    ...
    pipeline = [
        ...
        {"$match": {"foodNutrients.number": {"$in": nutrient_ids}}},
        {
            "$group": {
                "_id": "$fdcId",
...
def nutrition_summary_by_category(db, collection_name: str, out_collection: Optional[str] = None) -> List[Dict]:
    ...
    pipeline = [
        {"$unwind": "$foodNutrients"},
        {"$match": {"foodNutrients.number": {"$in": list(NUTRIENT_IDS.values())}}},
        {
            "$group": {
                "_id": {
                    "foodCategory": "$foodCategory",
                    "nutrientName": "$foodNutrients.name",
                },
                "avgValue": {"$avg": "$foodNutrients.amount"},
                "count": {"$sum": 1},
            }
        },
        {
...
def cross_collection_comparison(db) -> List[Dict]:
    ...
    for coll_name in RAW_COLLECTIONS:
        pipeline = [
            {"$unwind": "$foodNutrients"},
            {"$match": {"foodNutrients.number": {"$in": ["208", "203", "204", "269"]}}},
            {
                "$group": {
                    "_id": "$foodNutrients.name",
                    "avgValue": {"$avg": "$foodNutrients.amount"},
                    "minValue": {"$min": "$foodNutrients.amount"},
                    "maxValue": {"$max": "$foodNutrients.amount"},
                    "count": {"$sum": 1},
                }
            },
            {"$sort": {"_id": 1}},
...
def top_foods_by_nutrient(db, collection_name: str, nutrient_id: int, top_n: int = 10) -> List[Dict]:
    ...
    pipeline = [
        {"$unwind": "$foodNutrients"},
        {"$match": {"foodNutrients.number": nutrient_id}},
        {
            "$project": {
                "_id": 0,
                "fdcId": 1,
